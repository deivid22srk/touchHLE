...
pub(super) struct EAGLContextHostObject {
    ...
    renderbuffer_drawable_bindings: Arc<Mutex<HashMap<GLuint, id>>>,
...
+ (id)alloc {
    let host_object = Box::new(EAGLContextHostObject {
        gles_ctx: None,
        renderbuffer_drawable_bindings: Arc::new(Mutex::new(HashMap::new())),
        fps_counter: None,
        next_frame_due: None,
...
- (bool)renderbufferStorage:(NSUInteger)target
    ...
    retain(env, drawable);
    let host_obj = env.objc.borrow_mut::<EAGLContextHostObject>(this);
    let maybe_old_drawable = host_obj
        .renderbuffer_drawable_bindings
        .lock()
        .unwrap()
        .insert(renderbuffer, drawable);
